...
def update_coral_parameters():
	...
	#replaced above with below to avoid index error in year 0

    if opts.year < len(opts.yearly_total_coral_cover_df):
        old_coral_cover_perc = opts.yearly_total_coral_cover_df.iloc[opts.year-1]['total_coral_cover (%)']
        new_coral_cover_perc = opts.yearly_total_coral_cover_df.iloc[opts.year]['total_coral_cover (%)']
    else:
        print(f"Year {opts.year} is out of bounds for yearly_total_coral_cover_df with length {len(opts.yearly_total_coral_cover_df)}")
        old_coral_cover_perc = 0
        new_coral_cover_perc = 0
	...
	#below is safe fallback you can use inside update_coral_parameters() to prevent the .iloc indexing error - replaces above
    ...
    try:
        ...
        print(f"Year {opts.year} is out of bounds for yearly_total_coral_cover_df with length {len(opts.yearly_total_coral_cover_df)}")
        change_branching_cover = 0  # or np.nan if you prefer to track missing data
        change_foliose_cover = 0
        change_other_cover = 0
...
def run_yearly_change(PSD_df, Years):
	...
	for year in range(1, Years + 1):
		...
		if getattr(opts, "enable_sediment_exposure", True) and opts.add_suspended_sediment_lst is not None:
			opts.current_add_suspended_sediment = opts.add_suspended_sediment_lst[opts.year]
		...
		PCM_rates_ds = get_PCM_rates_after_DS_exp(PCM_rates, add_sedi_exp_per_year, opts.year, sedi_exp_PCM_coeff)      # coefficients from config.py
		
		from config import sedi_exp_growth_coeff
		#get_PCM_rates_after_DS_exp(PCM_rates, add_sedi_exp_per_year, year, sedi_exp_PCM_coeff)
		#get_GR_rates_after_SS_exp(growth_rate, add_sedi_exp_per_year, year, sedi_exp_growth_coeff)
		GR_ss = get_GR_rates_after_SS_exp(growth_rate, add_sedi_exp_per_year, opts.year, sedi_exp_growth_coeff)		
...
#for this version of the model I will include only the impact of ds
	...
	else:# if sediment exposure is not enabled 
		new_branching_pop = calculate_population_change(
    		opts.current_population_df['Branching'],
...
def get_PCM_rates_after_DS_exp(PCM_rates, add_sedi_exp_per_year, year, sedi_exp_PCM_coeff):
    ...
    add_deposited_sediment = add_sedi_exp_per_year.get(year, (0, 0))[1] # get deposited sediment for the year
    ...
    for coral_type in PCM_rates.columns:
        ...
        for base_rate in PCM_rates[coral_type]:
			...
			#adjusted_rate = base_rate + (coeff * add_deposited_sediment) #flip i cant decide if this must be /100 or not -come back to it 
            adjusted_rate = base_rate + (coeff * add_deposited_sediment)#/100 #put back and test
            adjusted_rate = max(0, min(adjusted_rate, 1))# Ensure PCM is always between 0 and 1
...
def get_GR_rates_after_SS_exp(growth_rate, add_sedi_exp_per_year, year, sedi_exp_growth_coeff):
    """
    Adjust growth rates based on suspended sediment exposure.
    """
    import pandas as pd
    import numpy as np  # Make sure NumPy is imported

    growth_rate_ss = pd.DataFrame(columns=growth_rate.columns)
    add_suspended_sediment = add_sedi_exp_per_year.get(year, (0, 0))[0]  # Get suspended sediment for the year
    for coral_type in growth_rate.columns:
        coeff = sedi_exp_growth_coeff.get(coral_type, 0)
		
		# Extract base rates as NumPy array
		# Growth rates are NumPy arrays, so we use vectorized operations instead of looping 
        base_rates = growth_rate[coral_type].values
		
		# Apply sediment impact
        adjusted_rates = base_rates + (coeff * add_suspended_sediment)
        adjusted_rates = np.maximum(0, adjusted_rates)  # Ensure no negative growth
		
		# Assign to output DataFrame
        growth_rate_ss[coral_type] = adjusted_rates
    return growth_rate_ss

# #### might need to remove below 
