...
def update_coral_parameters():
    ...
    def count_new_population_per_bin(year, binId, population, morphology, pcm_rate, wcm_rate, current_total_coral_cover):
        """
        ...
        # Use the new growth_rate DataFrame for year/bin/morphology-specific rates
        gr = growth_rate.loc[(year, binId), morphology]

        arr = np.array(np.linspace(lower_diameter, upper_diameter, 100))
        arr += gr * (1 - (current_total_coral_cover / opts.maximum_achievable_substrate_percentage) ** opts.growth_parameter)
...
def calculate_population_change(year, oldPop, growth_rate_df, pcm_rate, wcm_rate, available_substrate_percentage, coral_type):
    """
    ...
    growth_rate_df : pandas.DataFrame
        DataFrame containing growth rates indexed by (year, binId, morphology).
    pcm_rate : float
    ...
    for i in range(MaxBinId):
        # Use the new growth_rate DataFrame for year/bin/morphology-specific rates
        gr = growth_rate_df.loc[(year, i), coral_type]
        shrnk, rmn, grw, grw_grw = count_new_population_per_bin(
            year, i, oldPop[i], coral_type, pcm_rate[i], wcm_rate[i], opts.current_total_coral_cover
        )
        if i == 0:
...
def run_yearly_change(PSD_df, Years):
    ...
    using sediment-adjusted rates if enable_sediment_exposure is True.
    """
    ...
    # Choose correct rates based on sediment toggle
    if enable_sediment_exposure:
        growth_rate_df = growth_rate_SS
        PCM_rate_df = PCM_rates_DS
        # Use fertilisation_SS_rate_dict and settlement_rate_DS_dict as needed
    else:
        growth_rate_df = growth_rate
        PCM_rate_df = PCM_rates
        # Use default fertilisation and settlement rates
    ...
    for year in range(1, Years + 1):
        ...
        # If you use bleaching/cyclone, update PCM/WCM rates here
        PCM_rates_dhw = get_PCM_rates_after_dhw(PCM_rate_df, opts.current_dhw, branching_bleaching_rate, foliose_bleaching_rate, other_bleaching_rate)
        WCM_rate_cyc = get_WCM_rates_after_cyclones(WCM_rates, opts.current_cyc[0], opts.current_cyc[1])

        # Use sediment-adjusted rates for population change
        new_branching_pop = calculate_population_change(
            year, opts.current_population_df['Branching'], growth_rate_df, PCM_rates_dhw['Branching'],
            WCM_rate_cyc['Branching'], opts.available_substrate_percentage, 'Branching'
        )
        new_foliose_pop = calculate_population_change(
            year, opts.current_population_df['Foliose'], growth_rate_df, PCM_rates_dhw['Foliose'],
            WCM_rate_cyc['Foliose'], opts.available_substrate_percentage, 'Foliose'
        )
        new_other_pop = calculate_population_change(
            year, opts.current_population_df['Other'], growth_rate_df, PCM_rates_dhw['Other'],
            WCM_rate_cyc['Other'], opts.available_substrate_percentage, 'Other'
        )
