...
opts = CoralOptions()
# --- make sure opts has the user flags ---
try:
    from user_inputs import bleaching, cyclone
except Exception:
    # fallback if imported via config elsewhere; adjust if your project organises these differently
    pass
...
def initialize_coral():
    ...
    opts.year = 0
     ...
     # --- DHW only if bleaching enabled; else mirror old model (no list, zero effect) ---
    if getattr(opts, "bleaching", False):
    ...
    # --- Cyclone only if enabled; else mirror old model (no list, zero effect) ---
    if getattr(opts, "cyclone", False):
        opts.cyc_lst = create_cyclone_list(cyclone_years)
        opts.current_cyc = opts.cyc_lst[opts.year]  # usually a [cat, width] pair
    else:
        opts.cyc_lst = None
        opts.current_cyc = [0, 0]

    # Coral cover initialization (same as before)
    ...
    opts.unavailable_substrate_percentage = (
        opts.current_benthic_cover['macro_algae'] +
        opts.current_benthic_cover['rubble'] +
        opts.current_benthic_cover['sediment']
    )
...
def run_yearly_change(PSD_df, Years):
    ...
    for year in range(1, Years + 1):
        ...
        if opts.bleaching and opts.dhw_lst is not None:
            opts.current_dhw = opts.dhw_lst[opts.year]
        else:
            opts.current_dhw = 0  # no bleaching if disabled

        # Handle cyclones only if enabled
        if opts.cyclone and opts.cyc_lst is not None:
            opts.current_cyc = opts.cyc_lst[opts.year]
        else:
            opts.current_cyc = [0, 0]  # no cyclone if disabled

        # Calculate PCM rates — bleaching aware
        if opts.bleaching:
            PCM_rates_dhw = get_PCM_rates_after_dhw(
                PCM_rates,
                opts.current_dhw,
                branching_bleaching_rate,
                foliose_bleaching_rate,
                other_bleaching_rate
            )
        else:
            PCM_rates_dhw = PCM_rates.copy()  # no bleaching effect

        # Calculate WCM rates — cyclone aware
        if opts.cyclone:
            WCM_rate_cyc = get_WCM_rates_after_cyclones(
                WCM_rates,
                opts.current_cyc[0],
                opts.current_cyc[1]
            )
        else:
            WCM_rate_cyc = WCM_rates.copy()  # no cyclone effect# --- DHW: when disabled, force zero and DO NOT index dhw_lst ---
        if getattr(opts, "bleaching", False) and opts.dhw_lst is not None:
            opts.current_dhw = opts.dhw_lst[opts.year]
        else:
            opts.current_dhw = 0

        # --- Cyclone: when disabled, force [0,0] and DO NOT index cyc_lst ---
        if getattr(opts, "cyclone", False) and opts.cyc_lst is not None:
            opts.current_cyc = opts.cyc_lst[opts.year]
        else:
            opts.current_cyc = [0, 0]

        # --- CRITICAL: short-circuit the rate modifiers when flags are off ---
        if opts.current_dhw == 0:
            PCM_rates_dhw = PCM_rates
        else:
            PCM_rates_dhw = get_PCM_rates_after_dhw(
                PCM_rates,
                opts.current_dhw,
